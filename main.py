...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:5500"],  # Puedes poner el dominio de tu frontend aquí
...
async def handle_options():
    return JSONResponse(
        ...
        content={},
        headers={
            "Access-Control-Allow-Origin": "*",  # O pon el dominio específico de tu frontend
...
def eliminar_hoja_de_vida(hoja_id: int, db: Session = Depends(get_db)):
    return resumes_crud.eliminar_hoja_de_vida(db=db, hoja_id=hoja_id)from fastapi import FastAPI, Depends, HTTPException
from sqlalchemy.orm import Session
import crud.users as users_crud, crud.jobs as jobs_crud, crud.applications as applications_crud, crud.resumes as resumes_crud
import schemas
from database import SessionLocal, engine
